...
<portlet:renderURL var="shotDataURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>" >
 ...
 <form name="fm" action="<%= simulateShotDataURL.toString() %>" method="post">
	<fieldset>
	<%
		if (hasShotData){
	%>	
    ...
    <%} %>	
		...
		<aui:button-row>
			<aui:button type="button" name="upload" value="Upload Shot" onClick="javascript: openPopup();"/>
			<aui:button type="submit" name="simulate" value="Simulate" />
			<% if (!isRegistered) { %>
			<aui:button type="button" name="save" value="Save & Register" onClick="<%= addSubscriptionURL.toString() %>" disabled="<%= false %>" />
			<%} %>
		</aui:button-row>
